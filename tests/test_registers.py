...
from helpers.alphabets import ASCII_LOWERCASE_AND_DIGITS
...
@given(
    name=text(min_size=1, alphabet=ascii_uppercase),
...
@given(m=text(min_size=1, alphabet=ascii_uppercase), n=text(min_size=1, alphabet=ascii_uppercase))
...
@given(
    m=text(min_size=1, alphabet=ascii_uppercase),
...
@given(m=text(min_size=1, alphabet=ascii_uppercase), delta=integers())
...
@given(
    name=text(min_size=4, alphabet=ascii_uppercase),
    delta=one_of(just(-2), just(-1), just(+1), just(+2)),
)
